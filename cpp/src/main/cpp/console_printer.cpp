...
void ConsolePrinter::Print(Logger::LogRecord logRecord) {
  ...
  string color;
  if (record.level == Logger::Level::kInfo ||
      record.level == Logger::Level::kDebug) {
    color = RESET;
  } else if (record.level == Logger::Level::kWarn) {
    color = YELLOW;
  } else if (record.level == Logger::Level::kError) {
    color = RED;
  } else if (record.level == Logger::Level::kFatal) {
    color = BOLDRED;
  }
  cout << color << record.message_formatted << endl;
}
